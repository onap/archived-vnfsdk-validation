...
class ActiveRulesTableGenerator:
    ...
    def _create_rules_table(self, reqs_tag: str) -> ActiveRulesTable:
        ...
        for rule in active_rules:
            ...
            active_rules_table.add_entity(
                ActiveRuleEntity(
                    rule,
                    self._ged_description_from_yaml(yaml_description),
                    self._ged_product_from_yaml(yaml_description)
                )
            )
    ...
    @staticmethod
    def _ged_description_from_yaml(yaml_file: dict) -> str:
        return yaml_file['description'].replace("\n", " ")

    @staticmethod
    def _ged_product_from_yaml(yaml_file: dict) -> str:
